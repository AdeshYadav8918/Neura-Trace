...
# Private data directory for cache
...
class CVELookup:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        Args:
            api_key: Optional NVD API key (increases rate limits)
        """
    ...
    def search_by_keyword(self, keyword: str, max_results: int = 10) -> List[Dict]:
        ...
        Args:
            keyword: Search term (e.g., "Apache", "OpenSSH 7.4")
    ...
    def search_by_cpe(self, product: str, version: str = None) -> List[Dict]:
        ...
        cpe_match = f"cpe:2.3:a:*:{product.lower()}"
        if version:
            cpe_match += f":{version}"
    ...
    def get_cves_for_service(self, service_name: str, banner: str = "") -> Dict:
        """
        Get CVE information for a detected service
        
        Args:
            service_name: Name of the service (e.g., "Apache", "SSH")
            banner: Service banner text for version extraction
            
        Returns:
            Dictionary with CVEs and summary
        """
...
# Standalone function for quick lookups
def lookup_cves(service_name: str, banner: str = "", api_key: str = None) -> Dict:
    """
    Quick CVE lookup for a service
...
if __name__ == "__main__":
    ...
    # Test with Apache
    print("\n--- Testing Apache lookup ---")
    result = lookup.get_cves_for_service("Apache HTTP Server", "Apache/2.4.29")
    print(f"Found {result['cve_count']} CVEs for Apache")
    print(f"Risk Level: {result['risk_level']}")
    
    if result['cves']:
        print(f"\nTop CVE: {result['cves'][0]['cve_id']}")
        print(f"Score: {result['cves'][0]['cvss_score']}")
        print(f"Severity: {result['cves'][0]['severity']}")
